                       ...
                       int tolerance, int frameFreq)
{
    ...
    while(!X.isEmpty())
    {
        ...
        int distance = pow(int(origin.red - currPixel->red),2) + pow(int (origin.green - currPixel->green),2) + pow(int (origin.blue - currPixel->blue),2);
        if(!mop[make_pair(currX,currY)] && distance <= tolerance)
        {
            mop[make_pair(currX,currY)] = true;
