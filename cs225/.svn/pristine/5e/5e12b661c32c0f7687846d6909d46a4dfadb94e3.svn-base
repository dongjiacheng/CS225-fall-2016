...
length = 0;
curr = NULL;
head = NULL;    // Write this function based on mp3
}
...
{
  ListNode* in = new ListNode(ndata);
  length ++;
   if(head!=NULL)
   {
   in->next = head;
   ...
   head = in;
   in = NULL; // Write this function based on mp3
}
...
void List<T>::insertBack(const T& ndata)
{
    ...
    if (temp == NULL) {
        head = new ListNode(ndata);length++;
    } else {
        while (temp->next != NULL)
            temp = temp->next;
        ListNode* temp2  = new ListNode(ndata);
        temp -> next = temp2;
	length++;
    }
}
