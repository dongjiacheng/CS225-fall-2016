...
class Quadtree
{
  ...
  private:
    ...
    QuadtreeNode* buildTreehelper(PNG const &source, size_t x, size_t y, int resolution);
    ...
    int count (QuadtreeNode *subroot) const;
 
    int res;

    RGBAPixel getPixel(int x, int y, QuadtreeNode* subroot, int resolution) const;

    void clockwiseRotate (QuadtreeNode* node);

    void prune(QuadtreeNode* subroot, int tolerance);

    bool checkNode(QuadtreeNode* OriginRoot, QuadtreeNode* SubRoot, int tolerance);

    bool checkdiff(QuadtreeNode * node, QuadtreeNode * node2, int tolerance);

    int findbound(int ceilling, int floor, int numLeaves) const;

    int ceilling(int numLeaves, int tolerance) const;
//commite  
