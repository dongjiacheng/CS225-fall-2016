...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
  ...
  //  if (subroot == NULL)
   ...
   // }
    //result = find(subroot->children[first_larger_idx],key);
    //return result;
    if(!subroot->elements.empty()&&first_larger_idx < subroot->elements.size()&& subroot->elements[first_larger_idx] == key)
        return subroot->elements[first_larger_idx].value;
...
//    if(subroot->is_leaf)
//    {
//        auto elements_itr = subroot->elements.begin()+first_larger_idx;
//        subroot->elements.insert(elements_itr,pair);
//        subroot->children.insert(subroot->children.begin()+first_larger_idx,NULL);
//        if(subroot->elements.size()>= (order-1))
//            split_child(subroot,subroot->elements.size()/2);
//        return;
//    }
