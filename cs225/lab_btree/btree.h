...
size_t insertion_idx(const std::vector<T>& elements, const C& val)
{
 /**
    ...
    while(min < max)//rethink here
    {
        if(elements[mid] == val)
            return mid;
        else if(elements[max]< val)
            return max+1;
        else if (elements[min] > val)
            return min;
    ...
    */
    ...
    while(low < high){
        ...
        if(*mid == val){return (mid - elements.begin());}
        else if(*mid > val){high = mid;}
        else {low = mid+1;}
    }
    return(low - elements.begin());
