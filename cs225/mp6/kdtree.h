...
class KDTree
{
  public:
    ...
    void KDTreehelper(int start, int end, int d);
    /**
     * You do not need to modify this function. Its implementation is in
     *  kdtree_extras.cpp.
  ...
  private:
    ...
    void sort(int start, int end, int d);
    ...
    int distance(Point<Dim> child,Point<Dim> parent, int d) const;

    int radius(const Point<Dim> target,Point<Dim> currentBest) const;

    Point<Dim> findNearestNeighborhelper(const Point<Dim>& query, int start, int end, int d,Point<Dim> Curr, bool chang, bool hnb) const;
